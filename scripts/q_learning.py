...
# Initialisierungskonstanten der Motoren
ADDR_TORQUE_ENABLE, ADDR_GOAL_POSITION, ADDR_PRESENT_POSITION = 64, 116, 132

MOTOR_LIMITS = {
    DXL_ID_2: {'min': 1500, 'max': 2500},
    DXL_ID_3: {'min': 1000, 'max': 2000}
}


# Initialisierung der Kommunikation mit den Dynamixel-Motoren
portHandler = PortHandler(DEVICENAME)
packetHandler = PacketHandler(PROTOCOL_VERSION)


# Funktion, um alle Motoren zu aktivieren
...
# Funktion, um die Motor-Ports zu initialisieren
...
# Funktion, um eine Belohnung basierend auf der Encoder-Position zu erhalten
...
# Callback-Funktion für den linken Encoder
def left_encoder_callback(data):
    global left_encoder_value
    left_encoder_value = data.data


# Funktion, um den aktuellen Zustand aus den Motorpositionen abzuleiten
def discretize_motor_position(motor_pos, motor_min):
    return int((motor_pos - motor_min) / 200)
...
# Funktion zur Auswahl einer Aktion basierend auf Q-Werten
...
def execute_action_on_motors_and_get_feedback(action, state):
    if action == 0:
        adjust_motor_position(DXL_ID_2, 200)
    elif action == 1:
        adjust_motor_position(DXL_ID_2, -200)
    elif action == 2:
        adjust_motor_position(DXL_ID_3, 200)
    elif action == 3:
        adjust_motor_position(DXL_ID_3, -200)
...
def start_countdown():
    for i in range(5, 0, -1):
        ...
        time.sleep(1)
    update_oled_display("Running 5x5")
